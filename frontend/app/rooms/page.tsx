...
export default function LibraryRoomBooking() {
  ...
  // Helper function to format Date object to YYYY-MM-DD in LA timezone
  const formatDateKey = (dateObj) => {
    return dateObj.toLocaleDateString('sv-SE', { timeZone: 'America/Los_Angeles' });
  };

  // 1. Initialize state using local date format
  const [selectedDateKey, setSelectedDateKey] = useState(() => formatDateKey(new Date()));

  // 2. Update date carousel generation to use local format
  const dates = useMemo(() => {
    const list = [];
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const key = formatDateKey(d); // Matches getAvailableRooms expectations
  ...
  // 3. Pass selectedDateKey into getAvailableRooms inside fetchRooms
      ...
      if (slug === "all") {
        const results = await Promise.all(
          SINGLE_SLUGS.map(async (s) => {
            // Pass selectedDateKey here instead of null
  ...
  // Update this handler inside your LibraryRoomBooking component:
  const handleExternalBooking = (roomId:number) => {
    window.open(
      `https://berkeley.libcal.com/space/${roomId}?date=${selectedDateKey}`,
      '_blank',
      'noopener,noreferrer'
    );
  };
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl space-y-5">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto">
                {bookingRoom.slots.map((slot, index) => (
                  <div 
                    key={index}
                    className="py-2.5 px-3 rounded-xl border border-slate-200 text-slate-700 text-xs font-bold flex justify-between items-center bg-slate-50/50"
                  >
                    <span>{slot.time || 'Available Slot'}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="pt-2 flex gap-3">
              ...
              <button 
                onClick={() => handleExternalBooking(bookingRoom.id)}
